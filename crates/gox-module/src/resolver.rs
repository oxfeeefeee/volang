...
pub struct ModuleResolver {
    /// Project root directory (where gox.mod is located).
...
impl ModuleResolver {
    ...
    /// Returns the path to the gox.mod file.
    pub fn mod_file_path(&self) -> PathBuf {
        self.project_root.join("gox.mod")
    }

    /// Returns the path to the .goxdeps directory.
    ...
    ///
    /// This reads all gox.mod files recursively and builds a complete
    /// module closure. Returns an error if:
    /// - A required module is not found in .goxdeps
    ...
    pub fn compute_closure(&self, root_mod: &ModFile) -> ModuleResult<ModuleClosure> {
        ...
        while let Some((mod_path, version, chain)) = queue.pop() {
            ...
            // Find the module in .goxdeps
            let versioned_dir = self.find_module_dir(&mod_path, &version)?;

            // Parse the module's gox.mod
            let dep_mod_file_path = versioned_dir.join("gox.mod");
    ...
    fn find_module_dir(&self, module: &str, version: &str) -> ModuleResult<PathBuf> {
        // Expected path: .goxdeps/<module>@<version>/
...
mod tests {
    ...
    fn setup_test_project() -> (TempDir, ModuleResolver) {
        ...
        // Create gox.mod
        fs::write(
            temp.path().join("gox.mod"),
            "module github.com/test/project\n",
        ).unwrap();

        // Create a local package
        fs::create_dir_all(temp.path().join("util")).unwrap();
        fs::write(temp.path().join("util/util.gox"), "package util\n").unwrap();
    ...
    fn test_compute_closure_no_deps() {
        ...
        let mod_file = ModFile::parse_file(temp.path().join("gox.mod")).unwrap();
